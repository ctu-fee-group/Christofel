...
#pragma warning restore SA1405
        ...
        public PriorityQueue(int initialCapacity, IComparer<TPriority>? comparer)
        {
            if (initialCapacity < 0)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(initialCapacity), initialCapacity, "Out of range");
            }
        ...
        public int EnsureCapacity(int capacity)
        {
            if (capacity < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(capacity), capacity, "Negative capacity");
            }
...
#pragma warning disable SA1401
            ...
            void ICollection.CopyTo(Array array, int index)
            {
                ...
                if (array.Rank != 1)
                {
                    throw new ArgumentException("Multirank not supported", nameof(array));
                }

                if (array.GetLowerBound(0) != 0)
                {
                    throw new ArgumentException("Lowerbound ?", nameof(array));
                }
            ...
            public struct Enumerator : IEnumerator<(TElement Element, TPriority Priority)>
            {
                ...
                private bool MoveNextRare()
                {
                    ...
                    {
                        throw new InvalidOperationException("Enum failed verison");
                    }
